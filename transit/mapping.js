...
function parse() {
  ...
  // var Tstops = JSON.parse(str);

  /*
  function minDistance(value) {
    
    if(scheduleData["line"] == Tstops.Line) {
  ...
  for(i=0;i<(Tstops.length - 5);i++) {
    if(scheduleData["line"][i] == Tstops.Line) {
      linePathCoordinates[0] = new google.maps.LatLng(Tstops[i].Lat, Tstops[i].Lng); 
    }
  }
  ...
  if(scheduleData["line"][0] == 'blue') {
    color = '0000CC';
  } else if(scheduleData["line"][0] == 'orange') {
    color = 'FF850A';
  } else if(scheduleData["line"][0] == 'red') {
    color = 'FF0A0A';
  }
...
function getMyLocation() {
    ...
    if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            myLng = position.coords.Lngitude;
            renderMap();
        });
    }
    else {
